...
use crate::save_loader::ComponentType::{Bidirectional1, Bidirectional16, Bidirectional32, Bidirectional64, Bidirectional8, Custom, Deleted12, Deleted13, Deleted14, Deleted15, Deleted16, Error, Program, Program8_1, Program8_4, SolutionRom};
...
pub fn parse(input: &Vec<u8>, i: &mut usize) -> Option<SaveFile> {
    ...
    Some(SaveFile {
        ...
        hub_description: Box::default(),
        components: get_components(&input, &mut i)?,
        wires: get_wires(&input, &mut i)?
    })
}

fn get_components(input: &Vec<u8>, i: &mut usize) -> Option<Box<Vec<Component>>> {
    let mut components: Box<Vec<Component>> = Box::new(Vec::new());
...
fn get_component(input: &Vec<u8>, i: &mut usize) -> Option<Component> {
    ...
    if index.is_some() {
        component_type = [Bidirectional1, Bidirectional8, Bidirectional16, Bidirectional32, Bidirectional64][index.unwrap()];
    }
    ...
    Some(Component {
        ...
        selected_programs: (|| {
            let mut programs = Box::new(HashMap::new());
...
fn get_wires(input: &Vec<u8>, i: &mut usize) -> Option<Box<Vec<Wire>>> {
    let mut wires: Box<Vec<Wire>> = Box::new(Vec::new());
    let length = get_u64(&input, i)?;
    let mut j = 0u64;
    while j < length {
...
fn get_wire(input: &Vec<u8>, i: &mut usize) -> Option<Wire> {
    Some(Wire {
